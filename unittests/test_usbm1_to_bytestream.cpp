...
template <std::ranges::input_range Range> auto convert(std::uint8_t cable, Range const& range) {
  std::vector<std::byte> output;
  midi2::usbm1_to_bytestream m1tobs{cable};
  for (std::uint32_t m1 : range) {
    m1tobs.receive(m1);
...
TEST(USBM1ToByteStream, NoteOnCable1) {
  ...
  constexpr auto kk = 0x00U;
  constexpr auto vv = 0x00U;

  std::array events{std::uint32_t{(cable << 28) | (9U << 24) | (0x90 << 16) | (kk << 8) | vv}};
...
TEST(USBM1ToByteStream, ControlChangeCable10) {
  ...
  constexpr auto pp = 0x00U;
  constexpr auto vv = 0x00U;

  std::array events{std::uint32_t{(cable << 28) | (0xBU << 24) | (0xB0 << 16) | (pp << 8) | vv}};
  auto const actual = convert(cable, events);
...
TEST(USBM1ToByteStream, TimingClockCable3) {
  ...
  std::array events{std::uint32_t{(cable << 28) | (0xFU << 24) | (0xF8 << 16)}};
  auto const actual = convert(cable, events);
...
TEST(USBM1ToByteStream, SysExFourBytes) {
  constexpr auto cable = 0x3U;
  std::array events{std::uint32_t{(cable << 28) | (0x4U << 24) | (0xF0 << 16) | (0x00 << 8) | 0x01},
...
TEST(USBM1ToByteStream, SysExTwoBytes) {
  constexpr auto cable = 0x9U;
  std::array events{std::uint32_t{(cable << 28) | (0x6U << 24) | (0xF0 << 16) | (0xF7 << 8)}};
  auto const actual = convert(cable, events);
...
TEST(USBM1ToByteStream, SysExThreeBytes) {
  constexpr auto cable = 0x9U;
  std::array events{std::uint32_t{(cable << 28) | (0x7U << 24) | (0xF0 << 16) | (0x7F << 8) | 0xF7}};
  auto const actual = convert(cable, events);
